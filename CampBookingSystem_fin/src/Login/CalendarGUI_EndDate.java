...
public class CalendarGUI_EndDate extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CalendarGUI_EndDate frame = new CalendarGUI_EndDate();
			...
			for (int j = 0; j < 7; j++) {
				if(e.getSource() == jbtn1[i][j]) {
					if(day(year)[month][i][j] != null) {
						...
						bg.setEndDate(year + "-" + (month+1) + "-" + day(year)[month][i][j]);
						dispose();
						bg.main(null);
